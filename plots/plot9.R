plot9 <- df_special_v2  %>% filter(Cast_Seq==1) %>%

  ggplot()+

  background_image(coliseum) +

  geom_point(aes(x=x,y=y,color=f_special),size=0.5,alpha=0.8) +
  ...
  facet_grid(logNote3~f_special,scales="free") +
...
ggsave(paste0("./_img/plot_9.png"),plot9,
       width = 12, height=6,units="in",device = "png",dpi=300)
