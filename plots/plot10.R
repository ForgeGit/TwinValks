plot10 <- df_special_v2  %>% filter(Cast_Seq==1) %>%
  ...
  geom_point(aes(x=x,y=y,color=f_special),size=0.3,alpha=0.5) +
  ...
  facet_wrap(.~Raid_Color,scales="free",nrow=1)  +
  labs(title="", subtitle = "",
...
ggsave(paste0("./_img/plot_10.png"),plot10,
