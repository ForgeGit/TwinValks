plot2 <- df_special_v2  %>% filter(Cast_Seq==1) %>% #group_by(special) %>% summarise(n=n())%>%
  ...
  facet_wrap(.~f_logNote3,scales="free_y",nrow=4) +
  vivax_theme()+
...
ggsave(paste0("./_img/plot_2.png"),plot2,
       width = 2, height =5,units="in",device = "png",dpi=300)
