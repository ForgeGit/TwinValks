...
plot4 <- df_special_v2  %>% filter(Cast_Seq==1) %>% #group_by(special) %>% summarise(n=n())%>%
  ...
  facet_wrap(.~f_diff,scales="free_y",nrow=2) +
  vivax_theme()+
...
ggsave(paste0("./_img/plot_4.png"),plot4,
       width = 3, height =5,units="in",device = "png",dpi=300)
